...
11. Both simulation modes move forward in time; Prequel begins from a historical anchor and Sequel begins from the Main Campaign's current state or another forward anchor.
...
14. The admission gate is evaluated before branch persistence, storage selection, save creation, hook allocation, or scenario activation.
15. A Prequel reaching the Main Campaign must stop at the explicit convergence gate; it never merges automatically.
